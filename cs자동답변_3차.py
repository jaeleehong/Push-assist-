...
for sheet_name in xl.sheet_names:
    ...
    # 모든 요약 결과 수집 (중복 제거)
    unique_summaries = df['요약 결과_정리'].dropna().unique()
    all_summary_results.extend(unique_summaries)
...
# 중복 제거된 모든 요약 결과 출력
print("\n=== 전체 데이터에서 발견된 모든 요약 결과 ===")
unique_all_summaries = list(set(all_summary_results))
unique_all_summaries.sort()
for i, summary in enumerate(unique_all_summaries, 1):
    print(f"{i:2d}. {summary}")

# 욕설 관련 키워드가 포함된 항목들 찾기
print("\n=== 욕설/비속어 관련 키워드가 포함된 항목들 ===")
vulgar_keywords = ['욕설', '비속어', '90%', '80%']
for summary in unique_all_summaries:
    if any(keyword in summary for keyword in vulgar_keywords):
        print(f"발견: {summary}")

# 90% 버전이 있는지 특별히 확인
print("\n=== 90% 버전 확인 ===")
ninety_percent_found = [summary for summary in unique_all_summaries if '90%' in summary]
if ninety_percent_found:
    print("90% 버전 발견:")
    for item in ninety_percent_found:
        print(f"  - {item}")
else:
    print("90% 버전이 데이터에 없습니다.")
    
# 실제 데이터에서 정확한 매칭 확인
print("\n=== 정확한 매칭 확인 ===")
exact_matches = []
for summary in unique_all_summaries:
    if summary in target_list:
        exact_matches.append(summary)
        print(f"정확히 매칭됨: {summary}")

if not exact_matches:
    print("정확히 매칭되는 항목이 없습니다.")
    print("대상 리스트:")
